...
$wp_customize->add_setting( "tgtext-1", array( "default" => __( "In", "themesgenerator" ), "sanitize_callback" => "sanitize_text"	) );
...
$wp_customize->add_setting( "tgtext-1", array( "default" => __( "In", "themesgenerator" ), "sanitize_callback" => "sanitize_text"	) );$wp_customize->add_setting( "tgtext-2", array( "default" => __( "Insert your text here", "themesgenerator" ), "sanitize_callback" => "sanitize_text"	) );$wp_customize->add_setting( "tgtext-3", array( "default" => __( "Insert your text here", "themesgenerator" ), "sanitize_callback" => "sanitize_text"	) );$wp_customize->add_setting( "tgtext-4", array( "default" => __( "Insert your text here", "themesgenerator" ), "sanitize_callback" => "sanitize_text"	) );$wp_customize->add_setting( "tgtext-5", array( "default" => __( "Insert your text here", "themesgenerator" ), "sanitize_callback" => "sanitize_text"	) );$wp_customize->add_setting( "tgtext-6", array( "default" => __( "Insert your text here", "themesgenerator" ), "sanitize_callback" => "sanitize_text"	) );
$wp_customize->add_control( new WP_Customize_Control( $wp_customize, "customtgtextAll-1", array( "label"    => __( "Text 1", "themesgenerator" ), "section"  => "customtgtext-9999", "settings" => "tgtext-1", "type"     => "text" ) ));$wp_customize->add_control( new WP_Customize_Control( $wp_customize, "customtgtextAll-2", array( "label"    => __( "Text 2", "themesgenerator" ), "section"  => "customtgtext-9999", "settings" => "tgtext-2", "type"     => "text" ) ));$wp_customize->add_control( new WP_Customize_Control( $wp_customize, "customtgtextAll-3", array( "label"    => __( "Text 3", "themesgenerator" ), "section"  => "customtgtext-9999", "settings" => "tgtext-3", "type"     => "text" ) ));$wp_customize->add_control( new WP_Customize_Control( $wp_customize, "customtgtextAll-4", array( "label"    => __( "Text 4", "themesgenerator" ), "section"  => "customtgtext-9999", "settings" => "tgtext-4", "type"     => "text" ) ));$wp_customize->add_control( new WP_Customize_Control( $wp_customize, "customtgtextAll-5", array( "label"    => __( "Text 5", "themesgenerator" ), "section"  => "customtgtext-9999", "settings" => "tgtext-5", "type"     => "text" ) ));$wp_customize->add_control( new WP_Customize_Control( $wp_customize, "customtgtextAll-6", array( "label"    => __( "Text 6", "themesgenerator" ), "section"  => "customtgtext-9999", "settings" => "tgtext-6", "type"     => "text" ) ));
}
...
$wp_customize->add_setting( "tgimg-1", array( "sanitize_callback" => "esc_attr", "default" => "".get_template_directory_uri()."/images/n_logo-903.png"));
 ...
 */
require_once dirname( __FILE__ ) . '/inc/class-tgm-plugin-activation.php';
add_action( 'tgmpa_register', 'my_theme_register_required_pluginsmyslider1' );
/**
 ...
 */
function my_theme_register_required_pluginsmyslider1() {
	/**
	 * Array of plugin arrays. Required keys are name and slug.
	 * If the source is NOT from the .org repo, then source is also required.
	 */
...
/* https://wordpress.stackexchange.com/questions/270070/add-a-dropdown-to-theme-customizer */ 



add_action( "customize_register", "themegenchild_register_theme_customizermyslider1" ); function themegenchild_register_theme_customizermyslider1( $wp_customize ) { $wp_customize->add_panel( "tgsliders_options", array("priority" => 70, "theme_supports" => "","title" => __( "Sliders", "themesgenerator" ), "description" => __( "Set background image.", "themesgenerator" ), ) );
$nametoshow=str_replace("myslider","","myslider1");
$wp_customize->add_section( 'tgsliders_optionsmyslider1', 
 array(
    'title'       => __( "Slider $nametoshow", "themesgenerator" ), //Visible title of section
    'priority'    => 20, //Determines what order this appears in		"panel"    => "tgsliders_optionsmyslider1",	
    'capability'  => 'edit_theme_options', //Capability needed to tweak
    'description' => __('Select the slider of the front page.', 'themesgenerator'), //Descriptive tooltip
 ) 
);


$wp_customize->add_setting( 'sliderz_namemyslider1', //No need to use a SERIALIZED name, as `theme_mod` settings already live under one db record
 array(
...
if ( ! function_exists( 'tg_sliders_get_options' ) ) :
function tg_sliders_get_options() {
	$options = array( '' => __( 'None', 'themesgenerator' ) );
	...
	if ( class_exists( 'MetaSliderPlugin' ) ) {
		...
		foreach ( $sliders as $slider ) {
			$options[ 'meta:' . $slider->ID ] = __( 'MetaSlider: ', 'themesgenerator' ) . $slider->post_title;
		}
	}
	...
	if ( class_exists( 'SmartSlider3' ) ) {
		...
		foreach( $sliders as $slider ) {
			$options[ 'smart:' . $slider->id ] = __( 'Smart Slider: ', 'themesgenerator' ) . $slider->title;
		}
	}
...
$dropdown = tg_sliders_get_options( false );
...
$wp_customize->add_control( new WP_Customize_Control(
 ...
 'tgsliders_theme_namemyslider1', //Set a unique ID for the control
 array(
    'label'      => __( 'Select Slider Name', 'themesgenerator' ), //Admin-visible name of the control
    'description' => __( 'Your current created sliders', 'themesgenerator' ),
    'settings'   => 'sliderz_namemyslider1', //Which setting to load and manipulate (serialized is okay)
    'priority'   => 10, //Determines the order this control appears in for the specified section
    'section'    => 'tgsliders_optionsmyslider1', //ID of the section this control should render in (can be one of yours, or a WordPress default section)
    'type'    => 'select',
    'choices' => $dropdown
)
) );}
